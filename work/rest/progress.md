...
Stories 1–7 in `packages/web-rest/tests/unit/ux_pass2_test.drift` are acceptance tests. Story 6 tests dispatch-path body parse caching with `JsonHandle`. Caching is dispatch-only (no lazy self-cache through `&Request` — Drift lacks interior mutability for non-Copy types). Direct `body_json()` calls outside dispatch reparse correctly but do not cache.

| Story | Scenario | Type | Status | Remaining gap |
|-------|----------|------|--------|---------------|
| 1 | `scenario_malformed_json_body` | acceptance | `body_json()` → `malformed-json` tag | None |
| 2 | `scenario_validation_fields_email_age` | acceptance | `require_json_string()` / `require_json_int()` with field-entry contract | None |
| 3 | `scenario_path_int_query_default` | acceptance | `path_param()` → `validate_int()` | None |
| 4 | `scenario_query_int_parse_failure` | acceptance | `validate_int()` → field-entry contract + envelope | None |
| 5 | `scenario_three_field_accumulation` | acceptance | Validation collector: `merge_validation_error()` from real extractors/validators → `validation_error()` | None |
| 6 | `scenario_body_parse_caching` | acceptance | Dispatch-path caching: `body_json()` returns cached `JsonHandle`; body mutation proves no re-parse | None |
| 7 | `scenario_optional_query_default` | acceptance | `query_param()` → `validate_int()` string-first defaulting pattern | None |
