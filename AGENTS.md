...
- Finding handoffs must use Baton protocol 7. Before publishing or consuming a handoff, locate the locally deployed Baton distribution and read its `AGENTS-MAILBOX-PROTO.md` in full. The deployment supplies the executable and explicit absolute config path; never infer or hard-code either.
- This project's Baton identities are `web.reviewer` for the reviewer and `web.implementer` for the implementer. Resolve role-only instructions to those identities; do not substitute a participant from another domain.
